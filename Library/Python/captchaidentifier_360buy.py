...
class CaptchaIdentifier():
	...
	size = img.size[0]
	if img.size[0] == 60:
		blocks = [list(im.crop(b).getdata()) for b in [(20,4,29,15), (36,4,45,15), (47,4,56,15)]]
		...
		blocks = [list(im.crop(b).getdata()) for b in [(20,4,29,15),(31,4,40,15),(42,4,51,15), (58,4,67,15),(69,4,78,15)]]
		return '%s%s%s%s%s' % tuple([self.identify_number(b) for b in blocks])
	elif (img.size[0]==97):
		blocks = [list(im.crop(b).getdata()) for b in [(20,4,29,15),(31,4,40,15),(42,4,51,15),(53,4,62,15),(69,4,78,15),(80,4,89,15)]]
		return '%s%s%s%s%s%s' % tuple([self.identify_number(b) for b in blocks])
	elif (img.size[0]==110):
		blocks = [list(im.crop(b).getdata()) for b in [(20,4,29,15),(31,4,40,15),(42,4,51,15),(53,4,62,15),(64,4,73,15),(80,4,89,15),(91,4,100,15)]]
		return '%s%s%s%s%s%s%s' % tuple([self.identify_number(b) for b in blocks])
